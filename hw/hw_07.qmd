...
Transformator-Schaltung eines [dynamischen
Mikrofons](https://www.thomann.de/de/onlineexpert_topic_dynamische_mikrofone.html) @fig-hw7p1.
...
plt.title('Phase')
# plt.subplot(212)
plt.semilogx(f, np.rad2deg(np.unwrap(np.angle(H))))
plt.xlabel(r'$\log(f/f_0)$')
plt.ylabel(r'arg(H) in Grad')
plt.grid()
plt.show()
...
[Lautsprechers](https://www.thomann.de/de/onlineexpert_topic_lautsprecher.html). Dabei repräsentiert $\underline{Z}_1 =
R_1 + j \omega L_1$ die Impedanz der ruhenden Schwingspule und $\underline{Z}_M$, die Parallelschaltung aus $R_2$, $L_2$
und $C_2$, repräsentiert die dynamisch-mechanischen Eigenschaften des Systems.
