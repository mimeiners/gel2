...
::: {.callout-note}
Die Inhalte sind dem gleichnamigen Kapitel 9 von [@reisch2007] entnommen worden.
:::
...
Leistung. Spulen und Übertrager werden in der Elektronik hauptsächlich zum Aufbau von Stromversorgungen, galvanischen
...
Feldkonstante (Induktionskonstante) bezeichnet. Die Einheit der magnetischen Flußdichte ist Tesla (1T = 1Vs/m2). 
Durch Kernmaterialien mit ferromagnetischen oder ferrimagnetischen Eigenschaften wird in technischen Spulen und 
Übertragern häufig die magnetischen Flußdichte $\mathbf{B}$ erhöht. Durch die magnetische Polarisation dieser 
Materialien im Feld $\mathbf{H}$ ist die magnetische Flußdichte im Kern um die Permeabilitätszahl $\mu_r$ erhöht 
...
![$B(H)$-Hysteresekurve, Rayleigh-Schleife](../images/lec/lec8s1){#fig-lec8s1 width=600}
...
### Durchflutungs- und Induktionsgesetz
...
### Induktionskoeffizienten ausgewählter Leiterformen

Die Induktionskoeffizienten für nur einen Leiter, bspw. *Zylinderpulen*, *Ringkernspulen*, *Drahtringe* und *gedruckte
Spiulen* lassen sich aus der Energie des magnetischen Feldes
...
![(a) Zylinderspule, (b) Ringkernspule und \(c\) Drahtring](../images/lec/lec8s3){#fig-lec8s3 width=400}


![Gedruckte Spule (PCB)](../images/lec/lec8s4){#fig-lec8s4}
...
![Ersatzschaltung einer Spule.](../images/lec/lec8s2){#fig-lec8s2}

Abbildung @fig-lec8s2 zeigt eine Ersatzschaltung für eine Spule, die bei Kleinsignalaussteuerung verwendet
...
Parallel hierzu liegt die Wicklungskapazität $c_p$; dielektrische Verluste in der Isolation werden durch den zusätzlich 
...
### Impedanz, Eigenresonanz

Die Impedanz \underline{Z} der Spule errechnet sich aus der Ersatzschaltung @fig-lec8s2 zu
...
Der prinzipielle Verlauf des Scheinwiderstands $\underline{Z}$ als Funktion der Frequenz ist in Abb. @fig-lec8s2
...
![Dämpfungsperle](../images/lec/lec8s5){#fig-lec8s5}
...
der dominierende Gleichanteil keine Kernverluste verursacht; vgl. auch [@schlienz2020] Kap. 2 - Abwärtswandler.

![Schaltnetzteil mit Drossel-Abwärtswandler](../images/lec/lec8s6){#fig-lec8s6}


<!-- #### Power Management Integrated Circuits (PMICs) -->

<!-- ![PMICs](../images/lec/lec8sPMICs){#fig-lec8sPMICs width=400} -->
...
<!-- illustrated in @fig-lec8sPMICs PMICs can be integrated into complex integrated circuits (ICs) or -->
...
<!-- energies, such as solar and wind power, by enabling efficient power conversion and management." [@wicht2024] -->
...
![Haupt- und Streuflüsse im Transformator](../images/lec/lec8s7){#fig-lec8s4}

Abbildung @fig-lec8s4 zeigt den prinzipiellen Aufbau eines Transformators bzw. Übertragers mit einer 
...
![Schaltsymbol zweier gekoppelter Induktivitäten](../images/lec/lec8s8){#fig-lec8s8}
...
Im Fall idealer Kopplung ($\vert k \vert = 1$) verschwindet der zweite Term auf der rechten Seite von Gl. (49); das
Spannungsverhältnis ist dann 

$$
\frac{\underline{v}_1}{\underline{v}_2} = \frac{L_1}{L_2} = ü.
$$

Die Größe ü wird dabei als *Übertragungsverhältnis* bezeichnet. Zwischen den Zeigern der Ströme besteht nach Gl. (9.75)
der Zusammenhang 
...
![Idealer Übertrager. (a) Schaltsymbol und (b) Impedanztransformation mit idealem
Übertrager](../images/lec/lec8s9){#fig-lec8s9}

Abbildung @fig-lec8s9 (a) zeigt des Netzwerksymbol eines idealen Übertragers. Für diesen gelten die folgenden
...
Wird der Ausgang eines idealen Übertragers mit der Impedanz $\underline{Z}_L$ beschaltet (Abb. @fig-lec8s9 (b)), so gilt
$\underline{v}_2/i_2 = −\underline{Z}_L$. Für die Eingangsimpedanz $\underline{Z}_i$ des Übertragers folgt damit 
...
![T-Ersatzschaltung](../images/lec/lec8s10){#fig-lec8s10}

Die Transformatorgleichungen lassen sich in die in Abb. @fig-lec8s10 dargestellte T-Ersatzschaltung
...
![Ersatzschaltung eines unvollständig gekoppelten Übertragers](../images/lec/lec8s11){#fig-lec8s11}

Abbildung @fig-lec8s11 zeigt die Ersatzschaltung eines unvollständig gekoppelten $(k < 1)$, ansonsten verlustfreien
...
![(a) Parallel- und (b) Serienersatzschaltung eines verlustbehafteten
Übertragers](../images/lec/lec8s12){#fig-lec8s12 }


### Übertragungsfaktor

![Parallelersatzschaltung eines verlustbehafteten Übertragers mit beschaltetem
Ausgang](../images/lec/lec8s13){#fig-lec8s13}
...
Abb. @fig-lec8s13 gezeigte Ersatzschaltung herangezogen, wobei die durch $C_3$ und $C_4$ beschriebene
kapazitive Kopplung zwischen Primär- und Sekundärwicklung (vgl. Abb. @fig-lec8s12 (a)) vernachlässigt wird.

Der Ausgang des idealen Übertragers in der Ersatzschaltung Abb. @fig-lec8s13 ist mit der Impedanz
...
$\underline{v}'_1$ kann demnach die in Abb. @fig-lec8s14 dargestellte Ersatzschaltung herangezogen werden.

![Zusammenfassung und Transformation der Sekundärseite](../images/lec/lec8s14){#fig-lec8s14}
...
spezieller, verlustarmer Ferritwerkstoffe; vgl. mit [@zach2022], Kap. 15.
