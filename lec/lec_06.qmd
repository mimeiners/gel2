...
physikalischen Charakters enthält, die miteinander Energie austauschen können. Eine solche Anordnung wird als
...
* Zusammenschaltung von Induktivität $L$ und Kapazität $C$ heißt (idealer) Schwing- oder Resonanzkreis
  ...
  Beide bestimmen die *momentane Speicherenergie* in Kondensator und Spule:

\begin{align}
W_C(t) = W_{pot}(t) &= \frac{1}{2} C v_C^2(t) = \frac{1}{2} C v_C(0) \cos^2(\omega_0 t) \\
W_L(t) = W_{kin}(t) &= \frac{1}{2} L i_L^2(t) = \frac{1}{2} L\omega^2_0 C^2 v^2_C(0) \sin^2(\omega_0 t) 
= \frac{1}{2} C v^2_C(0) \sin^2(\omega_0 t)
\end{align}


* *Energiesatz: Erhalt der gespeicherten Gesamtenergie*

$$
W_{C}(t) + W_{L}(t)\lvert_{\omega_0} = \frac{1}{2} C v^2_C(0) \left(\cos^2(\omega_0 t) + \sin^2(\omega_0 t) \right) =
W_{ges} = const. 
$$

* Eigenfrequenz des idealen Schwingkreises, *Thomsonformel*
...
## Energiependel 
aus [@paul2017], Kap. 3.2.3

"Der ständige Energieaustausch zwischen Kondensator und Spule folgt aus den verkoppelten Strom-Spannungsbeziehungen: die 
Kondensatorspannung $u_C$ hat den Höchstwert bei Stromnulldurchgang und umgekehrt. Deshalb wechseln die Höchstwerte
ständig nacheinander.

![Energiependel im Schwingkreis](../images/lec/lec6energiependel){#fig-lec6energiep}

"Beide Energieformen pulsieren je mit doppelter Resonanzfrequenz $\omega_0$ zeitversetzt um $\pi/2$ (durch
$\cos^2(\omega_0 t)$) um einen zeitlichen Mittelwert [...].

"Damit tragen die mittleren Kondensator- und Spulenenergien je zur Hälfte zur gesamten Speicherenergie im idealen 
$LC$-System bei der Eigenfrequenz $\omega = \omega_0$ (nur hier!) bei:

* *Physikalische Resonanzbedingung*

$$
\overline{W_C(t)} = \overline{W_L(t)} = \frac{W_{ges}}{2}
$$

* *Gütemaß (Kreisgüte, Resonanzschärfe)*
  
  Zur Kennzeichnung der Verluste im Schwingungssystem eignet sich das Verhältnis der Speicherenergie (bei Resonanz) zur
  Verlustenergie je Periode. Wir definieren dafür die *Kreisgüte* $Q$ oder *Resonanzschärfe* $\varrho$

$$
\varrho = Q = \left . \frac{2 \pi \cdot \mbox{gesamte Speicherenergie}}
{\mbox{Verlustenergie je Periode}}\right\lvert_{\omega_0}
= \frac{\omega_0 \cdot \mbox{gesamte Speicherenergie}}{\mbox{Verlustleistung}}
$$

  Daraus folgt für *Reihen- und Parallelschwingkreis* gleichwertig:


  * Reihenkreis

$$
\varrho = \frac{2\pi W_{ges}}{T \cdot P} = \left . \frac{2\pi L I^2}{T \cdot R \cdot I^2}\right\lvert_{\omega_0} 
...
$$
\varrho = \left . \frac{2\pi C U^2}{T \cdot G \cdot U^2}\right\lvert_{\omega_0} 
= \frac{\omega_0 C}{G} = \frac{1}{G}\sqrt{\frac{C}{L}}
$$


## RLC-Reihenschwingkreis (aktiver Zweipol)

![RLC-Reihenschwingkreis](../images/lec/lec6rlc){#fig-lec6rlc}
...
\begin{align}
v_1(0) &= L \frac{di}{dt} + i\, R + \frac{1}{C} \int i dt  \\
...
\begin{align}
\underline{H}_v(j \omega) = \frac{\underline{V}_2}{\underline{V}_1} &= \frac{1}{1 + j \omega R C + \omega^2 LC} \\
...
* Güte $Q = \frac{\vert Q(\omega_0) \vert}{P(\omega_0)} = \frac{1}{\omega_0 RC} = \frac{\omega_0}{2 \delta}$
  ...
  "Für Schwingkreise mit einem Freiheitsgrad ist die *Güte* $Q$ definiert als das Verhältnis des Betrages der bei
  Resonanz in einem der beiden Blindelemente umgesetzten, also innerhalb des Schwingkreises pendelnden Blindleistung $Q$
  zur aufgenommenen Wirkleistung $P$.
  
  Die Bildung des Betrages der Blindleistung ist erforderlich, damit sowohl die in der Induktivi- tät als auch die in
  der Kapazität umgesetzte Blindleistung eingesetzt werden können und das Ergebnis unabhängig vom bei der
  Leistungsberechnung verwendeten Zählpfeilsystem ist." 

* Dämpfung / Verlustfaktor $d = \frac{1}{Q} = \frac{R}{\omega_0 L} = \frac{2 \delta}{\omega_0}$
  
  "Der Kehrwert der Güte ist der *Verlustfaktor*, der bei Schwingkreisen auch als die *Dämpfung* bezeichnet wird."

[@reisch2007][@wikipedia2025a] [@harriehausen2020]
:::
...
* Thomsonformel  
  $$
  \omega_0 = \underbrace{\frac{1}{\sqrt{L_r C_r}}}_{\mbox{Reihenschwingkreis}} 
  = \underbrace{\frac{1}{\sqrt{L_p C_p}}}_{\mbox{Parallelschwingkreis}}
  $$

* bei Resonanz erreichen Impedanz und Admittanz des Reihen- bzw. Parallelschwingkreises ein Minimum
...
&= R_r + j X(\omega) \\
\lvert\underline{Z}\lvert &= \sqrt{R_r^2 + X^2(\omega)} \quad \mbox{Betrag}\\
\arg{\underline{Z}} &= \arctan \frac{X(\omega)}{R_r} \quad \mbox{Phase} \\
Q &= \frac{\omega_0 L}{R} = \frac{1}{R}\sqrt{\frac{L}{C}} \quad \mbox{Güte}
\end{align}
:::
...
&= G_r + j B(\omega) \\
\lvert\underline{Y}\lvert &= \sqrt{G_p^2 + B^2(\omega)} \quad \mbox{Betrag}\\
\arg{\underline{Y}} &= \arctan \frac{B(\omega)}{G_p} \quad \mbox{Phase} \\
Q &= \frac{\omega_0 C}{G} = \frac{1}{G}\sqrt{\frac{C}{L}} \quad \mbox{Güte}
\end{align}
:::
...
![Eigenschaften von Schwingkreisen](../images/lec/lec6elemSchwing){#fig-lec6elemSchwing}
