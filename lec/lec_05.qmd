...
Das zugehörige Bode-Diagramm ist in @fig-bode-rc_tp zu sehen. Für Frequenzen $f << f_g$ gilt in guter Näherung 
$A_v \approx 1$ bzw. $a_v = 0 dB$; für Frequenzen $f >> f_g$ dagegen $A_v(f) \approx f_g/f$. Im Bode-Diagramm 
...
$a_v(f_g) = 20\,dB \cdot log(1/\sqrt{2}) \approx -3.01 \,dB$, während die Phasenverschiebung den Wert −45$^{}$
aufweist.
...
Um in LTspice das Bode-Diagramm des RC-Tiefpass zu erhalten, muss eine 
...
\frac{1}{\tau} \int_0^t v_1(t') \exp\left(-\frac{t - t'}{\tau} \right) dt'.
$$
...
so stellt sich am Ausgang für $t >> \tau = 15\,\mu s$ ein periodischer Spannungsverlauf mit dem Gleichanteil 2.5 V 
